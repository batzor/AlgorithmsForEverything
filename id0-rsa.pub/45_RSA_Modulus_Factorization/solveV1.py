...
modulus =      "00:cc:02:62:c3:76:4f:6d:1b:fa:48:5a:88:c0:56:\
...
e = 65537
private_exp = "00:87:80:3a:2b:0b:44:db:fa:8e:35:4a:74:b4:13:\
...
# since d < phi(n) and ed = k phi(n) + 1, e > k
k = 1
while k < e:
    # And k should divide d * e - 1
    while (d * e - 1) % k != 0:
        k += 1

    sum_of_pq = N - (d * e - 1) // k + 1
    D = sum_of_pq * sum_of_pq - 4 * N

    if(pow(isqrt(D), 2) != D):
       k += 1
       continue
    p = (sum_of_pq + isqrt(D)) // 2
    q = (sum_of_pq - isqrt(D)) // 2
    assert(p + q == sum_of_pq)
    print("k =", k)
    print("P =", p)
    print("Q =", q)
    min_prime = min(p, q)
    print("Answer is", min_prime % 100000007 )
    break
